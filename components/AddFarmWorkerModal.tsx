...
import { FarmWorker, FarmWorkerFormData } from '../types';

interface AddFarmWorkerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: FarmWorkerFormData) => void;
  worker: FarmWorker | null;
}

const AddFarmWorkerModal: React.FC<AddFarmWorkerModalProps> = ({ isOpen, onClose, onSave, worker }) => {
  const [formData, setFormData] = useState<FarmWorkerFormData>({
    name: '',
    phone: '',
    salary: 0,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-md m-4">
        <h2 className="text-2xl font-bold text-brand-brown mb-4">{worker ? 'تعديل بيانات العامل' : 'إضافة عامل جديد'}</h2>
...
export default AddFarmWorkerModal;